...
def train(data_folder=".", model_folder="runs"):
    ...
    train_files = [{keys[0]: img, keys[1]: seg} for img, seg in zip(images[:n_train], labels[:n_train])][:2]
    val_files = [{keys[0]: img, keys[1]: seg} for img, seg in zip(images[-n_val:], labels[-n_val:])][:1]
...
def infer(data_folder=".", model_folder="runs", prediction_folder="/home/claire/segment"):
    """
    ...
    logging.info(f"folder: {image_folder}")
    ...
    with torch.no_grad():
        for infer_data in infer_loader:   
            print(str(infer_data['image_meta_dict']['filename_or_obj'])) 
            if str(infer_data['image_meta_dict']['filename_or_obj']) != "['/home/claire/data/PNA_nifti/5M9TIPWC.nii.gz']": continue      
    ...
    submission_dir = os.path.join(prediction_folder, "to_submit_input1")
    if not os.path.exists(submission_dir):
        os.makedirs(submission_dir)
    files = glob.glob(os.path.join(prediction_folder, "COVID-19-AR-*", "*_ct"))
    for f in files:
        new_name = os.path.basename(f)
        new_name = new_name[len("COVID-19-AR-"):]
        new_name = new_name[: -len("_ct_seg_nii.gz")] + ".nii.gz"
